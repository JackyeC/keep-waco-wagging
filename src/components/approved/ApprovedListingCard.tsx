...
import { pendingConfig, type ApprovedListing } from "@/data/approvedListings";

export function ApprovedListingCard({ listing }: { listing: ApprovedListing }) {
  const href = `/approved/${listing.slug}`;
  const isPending = listing.evaluationStatus === "pending";
  ...
  return (
    <article className="card-panel flex flex-col overflow-hidden">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        {isPending ? (
          <>
            <p className="mt-3 text-[13px] font-medium tracking-[0.04em] text-wag-sage">
              {pendingConfig.note}
            </p>
        ...
        <div className="mt-auto pt-5">
          ...
          >
            ...
            {isPending ? "See what we know" : "See Full Dog-Friendly Report"}
          </Link>
        </div>
      </div>
    </article>
